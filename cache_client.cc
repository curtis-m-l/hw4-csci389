...
public:
    ...
    tcp::resolver resolver(ioc);    //TODO: Download a version of Boost with tcp
    ...
    Impl(std::string host, std::string port) {
        ...
        int const results = resolver_.resolve(host_, port_);
...
*/
    ...
    void set(key_type key, val_type val, size_type size) {
        ...
        std::string requestBody = "/" + key + "/" + val + "/" + size;
        http::request<http::string_body, std::unordered_map> req{http::verb::set, requestBody, HTTPVersion_};
    ...
    val_type get(key_type key, size_type& val_size) {
        ...
        std::string requestBody = "/" + std::to_string(key) + "/";
        http::request<http::string_body, std::unordered_map> req{http::verb::get, requestBody, HTTPVersion_};
        req.set(http::field::host, host_);
        req.set(http::field::user_agent, BOOST_BEAST_VERSION_STRING);
    ...
    bool del(key_type key) {
        ...
        http::request<http::string_body, std::unordered_map> req{http::verb::delete_, requestBody, HTTPVersion_};
        req.set(http::field::host, host_);
        req.set(http::field::user_agent, BOOST_BEAST_VERSION_STRING);
